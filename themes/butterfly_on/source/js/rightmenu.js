...
/** 使用浏览器菜单 */
var showCheameMenu = false;
/** 如果用户在 1 秒内右键两次，则打开浏览器菜单 */
var showTempCheameMenu = false;
/** 上次打开右键的时间，默认值是 1970-01-01 00:00:00.0000 */
var tempMenuTimer = 0;
...
function popupMenu() {
    ...
    window.oncontextmenu = function (event) {// 右键菜单入口点
        ...
        if (sessionStorage.getItem("showMenuMessage") != "1") {
            Snackbar.show({
                text: '如要使用浏览器右键菜单，请右键双击，谢谢。',
                pos: 'top-center',
                showAction: false
            });
            sessionStorage.setItem("showMenuMessage", "1");
        }
        if ((getNowTimeMills() - tempMenuTimer) < 500) {// 0.5 秒内右键两次，则打开浏览器菜单
            rmf.showRightMenu(false, 0, 0);
            return true;
        } else {
            tempMenuTimer = getNowTimeMills();
        }
